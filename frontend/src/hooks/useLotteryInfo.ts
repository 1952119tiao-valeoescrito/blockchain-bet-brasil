// Caminho: /src/hooks/useLotteryInfo.ts
'use client';
...
/**
 * Hook para buscar informações globais do contrato de loteria.
 * Dados que não dependem de uma rodada específica.
 */
export function useLotteryInfo() {
  // Busca o ID da rodada mais recente.
  const { data: rodadaAtualId, isLoading: isIdLoading } = useReadContract({
    address: contractAddress,
    abi: contractAbi,
    functionName: 'rodadaAtualId',
    // Otimização: Faz a busca a cada 30 segundos para manter atualizado.
    query: { refetchInterval: 30000 } 
  });

  // Busca o total de taxas acumuladas pela plataforma.
  const { data: taxasAcumuladas, isLoading: isTaxasLoading } = useReadContract({
    address: contractAddress,
    abi: contractAbi,
  ...
  return {
    contractAddress,
    rodadaAtualId: rodadaAtualId ? Number(rodadaAtualId) : 0,
    taxasAcumuladas,
    isLoading: isIdLoading || isTaxasLoading,
  };
}