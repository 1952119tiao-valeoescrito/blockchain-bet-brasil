...
// Componente do Modal de Escolha
function ChoiceModal({ 
  ...
  isOpen: boolean; 
  onClose: () => void; 
  onChoice: (choice: 'aposta' | 'invest-bet') => void 
}) {
  if (!isOpen) return null;

  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
      <div className="bg-slate-800 border-2 border-emerald-500/30 rounded-2xl p-8 max-w-md w-full mx-4">
        <h3 className="text-2xl font-bold text-white text-center mb-6">
          🎯 Escolha Seu Tipo de Aposta
        </h3>
        
        <div className="grid gap-4 mb-6">
          <button
            onClick={() => onChoice('aposta')}
            className="bg-emerald-500 hover:bg-emerald-400 text-white p-6 rounded-xl text-left transition-all duration-300 transform hover:scale-105 border-2 border-emerald-400/50"
          >
            <div className="font-bold text-lg mb-2">🎲 Aposta Regular</div>
            <div className="text-sm opacity-90">Valor: <strong>R$ 5,00</strong></div>
            <div className="text-sm opacity-90">Prêmio Máximo: <strong>R$ 50.000,00</strong></div>
            <div className="text-xs opacity-70 mt-2">Bônus: R$ 0,625 por zero pontos</div>
          </button>

          <button
            onClick={() => onChoice('invest-bet')}
            className="bg-purple-600 hover:bg-purple-500 text-white p-6 rounded-xl text-left transition-all duration-300 transform hover:scale-105 border-2 border-purple-400/50"
          >
            <div className="font-bold text-lg mb-2">💼 Invest-Bet Premium</div>
            <div className="text-sm opacity-90">Valor: <strong>R$ 1.000,00</strong></div>
            <div className="text-sm opacity-90">Prêmio Máximo: <strong>R$ 10.000.000,00</strong></div>
            <div className="text-xs opacity-70 mt-2">Bônus: R$ 125,00 por zero pontos</div>
          </button>
        </div>

        <button
          onClick={onClose}
          className="w-full py-3 text-slate-400 hover:text-slate-300 text-sm border border-slate-600 rounded-lg hover:border-slate-500 transition-colors"
        >
          Cancelar
        </button>
      </div>
...
const WalletConnector: React.FC = () => {
    ...
    const handleChoice = (choice: 'aposta' | 'invest-bet') => {
        setShowChoiceModal(false);
        if (choice === 'aposta') {
            window.location.href = '/apostas';
        } else {
            window.location.href = '/invest-bet';
        }
    };
    ...
    return (
        <div className="min-h-screen flex flex-col bg-gradient-to-br from-slate-900 to-slate-800 text-slate-100">
            ...
            {/* Cabeçalho */}
            <header className="w-full bg-slate-800 shadow-md sticky top-0 z-40 border-b border-emerald-500/30">
                <div className="container mx-auto flex justify-between items-center p-4">
                    ...
                    <nav className="hidden lg:flex items-center gap-8 font-semibold text-lg">
                        ...
                        <Link href="/invest-bet" className="text-slate-300 hover:text-purple-400 transition-colors duration-200">
                            💼 Invest-Bet
                        </Link>
                    ...
                    <div className="w-full flex flex-col items-center justify-center gap-12">
                        ...
                        <section className="w-full max-w-5xl flex flex-col items-center justify-center text-center gap-6">
                            ...
                            <p className="text-xl md:text-2xl text-slate-300 max-w-3xl">
                                Prepare-se para uma experiência de apostas descentralizada como nunca visto antes.
                        ...
                        <section className="w-full max-w-6xl mt-16 p-8 bg-slate-800/70 rounded-3xl shadow-2xl border-2 border-emerald-600">
                            ...
                            <div className="mt-16 text-center">
                                ...
                                >
                                    {isConnected ? 'Fazer Aposta!' : 'Conectar para Apostar!'}
